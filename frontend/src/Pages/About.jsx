...
import { getTimelineUpdates } from "../lib/api.js";
...
const About = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getTimelineUpdates();
        if (data) {
          ...
          const grouped = data.reduce((acc, update) => {
            ...
            acc[date].updates.push(update);
            return acc;
          }, {});
          
          // Convert object to array and sort if needed (assuming backend handles order)
          setRecentUpdates(Object.values(grouped));
        }
      } catch (error) {
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-cyan-900 dark:text-cyan-300 font-medium">
                      Updates are managed through the admin dashboard and automatically displayed here.
                    </p>
                  </div>
                </div>
              </div>
